...
must_haves:
  truths:
    ...
    - "RecipePickerDialog returns MealSelection with RecipeId or CustomMealName (not both)"
    - "MealSelection is a public nested class in RecipePickerDialog for component reuse"
  ...
  artifacts:
    - path: "src/FamilyCoordinationApp/Components/MealPlan/RecipePickerDialog.razor"
      ...
      min_lines: 60
      contains: "public class MealSelection"
  ...
  key_links:
    ...
    - from: "src/FamilyCoordinationApp/Components/MealPlan/RecipePickerDialog.razor"
      to: "MealSelection result"
      via: "Submit returns MealSelection"
      pattern: "MudDialog\\.Close\\(DialogResult\\.Ok\\(.*MealSelection"
...
- MudAutocomplete<Recipe>
  ...
  - SearchFunc="SearchRecipes" (async, takes string value only - NO CancellationToken)
...
**IMPORTANT - MealSelection as nested public class:**
Define MealSelection as a public nested class at the bottom of the @code block so consuming components can reference it as `RecipePickerDialog.MealSelection`:
...
// IMPORTANT: MudAutocomplete SearchFunc does NOT provide CancellationToken
// Signature must be: Task<IEnumerable<T>> SearchFunc(string value)
private async Task<IEnumerable<Recipe>> SearchRecipes(string value)
{
    var recipes = await RecipeService.GetRecipesAsync(HouseholdId, value);
    return recipes;
}
...
File contains "public class MealSelection" as nested class
SearchRecipes signature has only string parameter (no CancellationToken)
  </verify>
  <done>
RecipePickerDialog allows recipe search or custom meal entry and returns MealSelection result via nested public class
  </done>
</task>
...
4. RecipePickerDialog.MealSelection is a public nested class
5. SearchRecipes has correct signature (string only, no CancellationToken)
6. WeeklyCalendarView uses CSS Grid with 7 columns
7. WeeklyListView uses MudExpansionPanels for day grouping
8. Both views use MealSlot component
</verification>

<success_criteria>
- RecipePickerDialog returns MealSelection with RecipeId or CustomMealName
- MealSelection is public nested class accessible as RecipePickerDialog.MealSelection
