...
user_setup:
  - service: google-oauth
    ...
    dashboard_config:
      ...
      - task: "Add authorized redirect URI: https://family.heathdev.me/signin-google (prod)"
        location: "OAuth client configuration"
...
<task type="auto">
  <name>Task 2: Create custom authorization requirement and handler for email whitelist</name>
  <files>
    src/FamilyCoordinationApp/Authorization/WhitelistedEmailRequirement.cs
    src/FamilyCoordinationApp/Authorization/WhitelistedEmailHandler.cs
    src/FamilyCoordinationApp/Program.cs
...
```
  ...
  <done>WhitelistedEmailRequirement and WhitelistedEmailHandler created. FallbackPolicy applies whitelist check to all authenticated requests.</done>
</task>

<task type="auto">
  <name>Task 3: Create login, logout, and access denied pages</name>
  <files>
    src/FamilyCoordinationApp/Components/Pages/Account/Login.razor
    src/FamilyCoordinationApp/Components/Pages/Account/Logout.razor
    src/FamilyCoordinationApp/Components/Pages/Account/AccessDenied.razor
  ...
  <action>
Create Components/Pages/Account/ directory with auth pages.
...
**Create minimal API endpoint for Google challenge** (add to Program.cs before app.Run()):
```csharp
...
Should show Login.razor, Logout.razor, AccessDenied.razor.
  ...
  <done>Login, Logout, and AccessDenied pages created. MainLayout shows user email and logout link. Auth endpoints configured in Program.cs.</done>
</task>
