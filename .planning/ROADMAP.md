...
**Status**: Delivered 2026-07-06 — gated-autonomous build of the validated, council-hardened 8-WP / 6-wave spec (`D:\Development\data\outputs\workshops\fca-chores-v1.3-multiroom\`) in ONE PR (`feat/chores-v1.3-multiroom`). Expand→migrate→switchover→contract: `ChoreRoom` join + backfill (WP-01), membership-set write path + validation (WP-02), room-delete membership removal (WP-03), board `roomIds[]` + per-room rollup fan-out + M9 contract lockstep (WP-04), island multi-chip + `roomGroups` fan-out (WP-05), multi-select pickers (WP-06), digest per-room fan-out (WP-07), and the contract step dropping `Chore.RoomId` (WP-08). Full suite 868 green (incl. real-Postgres migration chain + household-dedup); `:8080` browser-verified. Rode along: equity-heading window honesty fix.
**Depends on**: Phase 12
