...
must_haves:
  ...
  artifacts:
    - path: "src/FamilyCoordinationApp/Components/Recipe/IngredientEntry.razor"
      provides: "Ingredient entry with parsing"
...
<objective>
Create ingredient entry component with natural language parsing and drag-drop reordering.

Purpose: Enable the hybrid ingredient entry experience where users type "2 cups flour" and get structured fields.
Output: IngredientEntry component for parsing, IngredientList component for display/reorder.
</objective>
...
<task type="auto">
  <name>Task 2: Create IngredientEntry component with parsing</name>
  <files>
    src/FamilyCoordinationApp/Components/Recipe/IngredientEntry.razor
  </files>
  <action>
...
@code {
    ...
    // Default categories - will be replaced with database lookup
    private static readonly string[] Categories = { "Meat", "Produce", "Dairy", "Pantry", "Spices", "Frozen", "Bakery", "Beverages", "Other" };
    ...
    private static string SuggestCategory(string ingredientName)
    {
...
```
  ...
  <done>IngredientEntry component created with parsing, autocomplete, and category suggestion</done>
</task>

<task type="auto">
  <name>Task 3: Create IngredientList component with drag-drop and undo</name>
  <files>
    src/FamilyCoordinationApp/Components/Recipe/IngredientList.razor
  </files>
  <action>
...
3. IngredientEntry.razor exists with parsing and autocomplete
4. IngredientList.razor exists with drag-drop and undo delete
5. Mobile drag-drop polyfill script added to App.razor
</verification>
