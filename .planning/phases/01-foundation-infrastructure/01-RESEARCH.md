...
| Docker Compose | Kubernetes | Compose simpler for single-server deployment (thedarktower) |
...
server {
    listen 80;
    server_name family.heathdev.me;
...
4. **Google OAuth client separation (dev vs prod)**
   ...
   - Recommendation: Use separate OAuth clients (one for localhost:*, one for family.heathdev.me) to avoid accidental production auth with dev credentials. Minimal setup overhead.
...
5. **Docker Compose override usage (separate dev/prod configs)**
   ...
   - What's unclear: Best practice for single-server deployment where dev and prod are same environment (thedarktower)
