...
- **Non-empty /api error bodies** — now guaranteed by the pipeline, not by discipline: `/api` is branched past `UseStatusCodePagesWithReExecute` to `UseStatusCodePages`, which backfills a JSON `{message}` on any bodiless 4xx and preserves the status (PR #90; guard `ApiErrorBodyTests`). Still write your own message — the backfill is deliberately generic. **Exception half closed too** (PR #98): an `/api` `UseExceptionHandler` branch (`ApiExceptionResponse`) keeps a thrown `BadHttpRequestException`'s status and answers JSON 500 on any other unhandled throw — never a stack trace or HTML page on `/api`, in any environment.
