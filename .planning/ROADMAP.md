...
- [x] **Phase 13: Chores v1.3 — Multi-room chores** - COMPLETE (2026-07-06, one PR): one chore belongs to 0..N rooms (M:N)
...
- [x] **Phase 16: Chores v1.6 — Snooze & set-next-due** - SHIPPED (PR #47, 2026-06-22): a tz-resolved `Chore.SnoozedUntil` floor under next-due (the "not today" lever) — suppresses a chore until its date, skips a Fixed cadence slot it precedes, reschedules a OneOff, and gives recurring chores a first-due-on-create (the burst fix). Excluded from all four attention surfaces (board/home/equity/digest). `PATCH /api/chores/{id}/snooze` + snooze menu/chip + edit-sheet "next due" / add-sheet "first due". Spec: `workshops/chores-snooze-and-reschedule`. Built as the Blazor island, then carried into the SvelteKit SPA by the de-Blazor flip (#65); prod-confirmed working 2026-07-05.
- [x] **Phase 15: Chores v1.5 — Equity rework (invisible labor)** - COMPLETE (all faces shipped). The *temporal* face — chore-history surface (ledger + logbook) — **SHIPPED 2026-07-05 (PR #70, live in prod)**. The *equity-measurement* face (R1–R3) **SHIPPED 2026-06-15 (PR #43, merge `9d42ac5`, 6 WPs)** — discovered already-built during the 2026-07-05 re-grounding after the docs were never flipped at build time — and **prod-VALIDATED 2026-07-05** (founding case reads correctly: Natalie — 49% of board setup yet ~3% on the old physical-points lens — now leads the planning lanes; Reduced/Minimal capacity tiers exercised by both adults). Shipped: (1) rename→"Physical board-load — this week"+demote, (2) capacity-aware physical share (`PhysicalCapacityTier` + `ExpectedSharePct`), (3) planning footprint lanes (un-blended labeled tallies). **R4 SHIPPED 2026-07-06 (PR #74, merge `33350ac`)** — the gap-driven suggestion was killed by the framing grill (it structurally targets the capacity-limited planner in every tier config, i.e. the forbidden "who's behind" ranking) and reshaped to **R4′ capacity-fit**: match the chore's declared weight to the viewer's own self-set capacity — chore-subject, self-only, pull-only, zero new math — as v0 "Quick wins" (weight gauge + within-section quick-first sort) + v1 "Fits me" (a self-only whitelist chip that filters the pile to fitting tiers). Still deferred: the MealPlanEntry creator migration (no meal-plan signal). KB: `chores-equity-rework-phase-15-grill-resolved-prod-probe-verdict-build`.
...
### Phase 13: Chores v1.3 — Multi-room chores (COMPLETE)
**Goal**: Let a single chore belong to more than one room (e.g. "make all the beds", "dust all the rooms downstairs").
**Status**: Delivered 2026-07-06 — gated-autonomous build of the validated, council-hardened 8-WP / 6-wave spec (`<workspace>/data/outputs/workshops/fca-chores-v1.3-multiroom/`) in ONE PR (`feat/chores-v1.3-multiroom`). Expand→migrate→switchover→contract: `ChoreRoom` join + backfill (WP-01), membership-set write path + validation (WP-02), room-delete membership removal (WP-03), board `roomIds[]` + per-room rollup fan-out + M9 contract lockstep (WP-04), island multi-chip + `roomGroups` fan-out (WP-05), multi-select pickers (WP-06), digest per-room fan-out (WP-07), and the contract step dropping `Chore.RoomId` (WP-08). Full suite 868 green (incl. real-Postgres migration chain + household-dedup); `:8080` browser-verified. Rode along: equity-heading window honesty fix.
**Depends on**: Phase 12
**Source**: Production feedback #6 (in-app Feedback table, 2026-05-31).
**Why it was a phase, not a quick win**: `Chore.RoomId` was a single nullable FK. Multi-room meant a **many-to-many** `Chore ↔ Room` (join table + migration) that rippled through:
...
**Source**: in-app feedback (2026-06-14). Spec: `<workspace>/data/outputs/workshops/chores-v1.4-views-subtasks/`.
**Two capabilities**:
...
### Phase 15: Chores v1.5 — Equity rework / invisible labor (COMPLETE — measurement slice + R4′ capacity-fit + chore-history surface all shipped)
...
**Why it's a phase**: the pre-rework Equity lens counted only physical board completions (`EffortPoints`), which undercounts members whose contribution is planning/coordination/mental-load — invisible to the board. Rework needed (1) **new contribution dimensions** beyond physical completions and (2) **mechanisms that drive toward equity** (capacity-aware suggestion), not just a displayed distribution.

**Equity-measurement slice — SHIPPED 2026-06-15 (PR #43, merge `9d42ac5`, 6 WPs) + prod-VALIDATED 2026-07-05**: rungs R1–R3 of the grill-resolved plan (KB `kb-save://2026-06-15/chores-equity-rework-phase-15-grill-resolved-prod-probe-verdict-build`; workshop `data/outputs/workshops/chores-v1.5-equity-measurement/`) — **R1** lens rename ("Board load" chip / "Physical board-load — this week" heading, lens id `equity` kept), **R2** capacity-aware expected share (`PhysicalCapacityTier` Full/Reduced/Minimal, self-set `PATCH /api/chores/me/capacity`, `ExpectedSharePct` on `ChoreEquityCalculator`; raw `SharePct`/`EqualSharePct` untouched — digest byte-identical), **R3** planning-footprint lanes (`ChorePlanningCalculator`: chores set up / recipes / list items / hand-offs, all-time, un-blended, neutral). Survived the de-Blazor flip intact (`frontend/app/src/lib/chores/lib/components/EquityBoard.svelte`). Built same-day gated-autonomous but the docs/quest were never flipped — discovered shipped during the 2026-07-05 re-grounding (`data/outputs/reviews/fca-phase15-equity-regrounding/REGROUNDING-MEMO.md`). **Validation confirmed in prod (operator, 2026-07-05)**: the founding case reads correctly — Natalie leads the planning lanes — and both household adults have exercised Reduced/Minimal capacity tiers. **R4 SHIPPED 2026-07-06 (PR #74, merge `33350ac`)** — reshaped to **R4′ capacity-fit** after the framing grill (`data/outputs/reviews/fca-phase15-r4-framing/R4-FRAMING-VERDICT.md`) killed the gap-driven suggestion: `SharePct − ExpectedSharePct` structurally targets the capacity-limited planner (Natalie) in *every* tier config — the forbidden "who's behind" ranking with a euphemism. What shipped instead matches the *chore's* declared weight to the *viewer's own* self-set capacity tier — chore-subject, self-only, pull-only, **zero new data or math** (both signals already ship). One backend field (`CallerCapacityTier` on the board DTO — additive init-only prop, one query) + two surface WPs: **v0 "Quick wins"** (effort weight-gauge on the card + a within-section quick-first sort) and **v1 "Fits me"** (a self-only chip, rendered only for a Reduced/Minimal caller, that filters the up-for-grabs pile to fitting tiers with a "N bigger lifts set aside · Show them" escape hatch + the provenance note "…never to history"). Doctrine-clean: no gap signal in the pile path, no push, no persistence, chore-subject copy; the whitelist gate is unit-tested (via a new vitest harness — the first frontend tests — now wired into CI as a `Frontend Check & Test` job). Spec: `data/outputs/workshops/fca-phase15-r4-capacity-fit/spec.md`. The old "equity heading says 'this week' on the All-time window" nit was a false alarm — the heading is already window-conditional (WP-04 dropped as a confirmed no-op).
...
| 15. Chores v1.5 (Equity rework / invisible labor) | R1–R3 (6 WPs) + R4′ (3 WPs) | **COMPLETE** — all faces shipped | **PR #43** (2026-06-15, `9d42ac5`) measurement slice, prod-validated 2026-07-05; **history surface PR #70** (2026-07-05); **R4′ capacity-fit PR #74** (2026-07-06, `33350ac`) — Quick wins + Fits me |
...
---
*Created: 2026-01-22*
*Last updated: 2026-07-06 (Phase 15 R4 SHIPPED — reshaped to **R4′ capacity-fit**, merged as PR #74 → master `33350ac`; all 5 CI checks green, incl. a new `Frontend Check & Test` job that now gates the SPA — svelte-check + vitest — the frontend previously had no CI job. **Phase 15 is now COMPLETE, and with it the entire planned FCA roadmap (Phases 1–16; 8–9 deprecated) — every phase is `[x]`.** Trust the Spine campaign "Family Coordination App" / quests `598a1d49` + `38c77cc4` (both done) over this snapshot for live work-state.)*
