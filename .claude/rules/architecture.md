...
- **Reads:** every `ITenantEntity` carries a named global query filter `"Tenant"` (`ApplicationDbContext.ApplyTenantFilter`), which scopes queries, `Include` navigations and `ExecuteUpdate`/`ExecuteDelete`. The tenant comes from the scoped `ITenantContext`: `CallerTenantMiddleware` sets it for `RequireTenant()` groups. A tenant query with no tenant **throws `TenantNotSetException`**, never returns empty or everything.
- **Writes** are checked in `SaveChanges` (`Data/ApplicationDbContext.Writes.cs`): an unset `HouseholdId` on an added row is stamped from the tenant; a row of another household, a changed `HouseholdId`, or a forged surrogate-key row throws `CrossTenantWriteException`; `CreatedAt` is stamped on insert when unset; `Tenancy:EnforceWrites` is the kill switch. The one sanctioned cross-household write is `context.Tenant.AllowCrossTenantWrite(reason)` at invite accept, pinned by `TenantWriteArchitectureTests`, which also bans a `SetProperty` on `HouseholdId` (`ExecuteUpdate` bypasses `SaveChanges`). Both tenancy exceptions derive from `Exception`, not `InvalidOperationException`: never catch them as "not found". That covers the write step's own refusals; on a composite-key type (the key includes `HouseholdId`), EF's key guard refuses a changed `HouseholdId` first, as an `InvalidOperationException`, so that write is blocked but not soak-visible.
...
- **System work** with no caller (digest, calendar feed, setup, admin approve/create, the dev seed, the login profile write) runs inside `context.Tenant.RunAs(householdId)`; the calling files and their call counts are pinned.
- **Never hold `RunAs` scopes for different households concurrently within one DI scope, nor an `AllowCrossTenantWrite` scope beside a concurrent save (a sibling save inherits the exemption).** `RunAs` pushes onto the one scoped tenant (no `AsyncLocal`, MN10), so parallel branches (`Task.WhenAll` over households) would mix tenants and trip its LIFO check. Give each branch its own `IServiceScopeFactory.CreateScope()`. One `RunAs` around a `WhenAll` for a single household (the calendar feed) is fine.
