...
must_haves:
  truths:
    ...
    - "User can navigate between weeks and data reloads automatically"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/FamilyCoordinationApp/Components/Pages/MealPlan.razor"
      ...
      pattern: "<MudHidden"
    - from: "src/FamilyCoordinationApp/Components/Pages/MealPlan.razor"
      to: "MealPlanNavigation"
      via: "WeekStartDateChanged event handler"
      pattern: "WeekStartDateChanged=\"@HandleWeekChanged\""
---
...
@code {
    ...
    private async Task HandleSlotClick((DateOnly Date, MealType MealType) slot)
    {
        ...
        if (!result.Canceled && result.Data is RecipePickerDialog.MealSelection selection)
        {
...
1. Use explicit `WeekStartDateChanged="@HandleWeekChanged"` NOT `@bind-WeekStartDate`
   - @bind would work for the value but wouldn't call our reload logic
   - We need HandleWeekChanged to update _weekStartDate AND call LoadMealPlanAsync()
2. HandleWeekChanged is async Task, updates state, then reloads data
3. Use RecipePickerDialog.MealSelection (nested public class) for type-safe result
4. Both views are always rendered but one is hidden via CSS - this avoids flash on initial load
5. Reload entries after any modification (add or remove)
  </action>
  <verify>
Build succeeds: `dotnet build src/FamilyCoordinationApp`
File has @page "/meal-plan" route
File uses MudHidden for responsive switching
File uses WeekStartDateChanged="@HandleWeekChanged" (not @bind)
HandleWeekChanged calls LoadMealPlanAsync
  </verify>
  <done>
MealPlan page shows calendar on desktop, list on mobile, handles all CRUD operations via dialogs, week navigation reloads data
  </done>
</task>
...
**Week Navigation:**
18. Click left arrow - verify previous week loads (not just displays - check that any meals you added are gone)
19. Click right arrow - verify next week loads
20. Click "Today" button - verify returns to current week with your meals
...
2. MealPlan.razor uses explicit WeekStartDateChanged handler (not @bind)
3. HandleWeekChanged updates week AND calls LoadMealPlanAsync
4. MealPlan.razor has responsive MudHidden switching
5. RecipeDetailDialog shows full recipe info
6. NavMenu has Meal Plan link
7. All meal planning requirements (MEAL-01 through MEAL-06) are functional
</verification>
...
- Week navigation works correctly AND reloads data
