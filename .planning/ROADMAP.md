...
- [ ] **Phase 15: Chores v1.5 — Equity rework (invisible labor)** - TWO FACES, BOTH SHIPPED; only R4 open. The *temporal* face — chore-history surface (ledger + logbook) — **SHIPPED 2026-07-05 (PR #70, live in prod)**. The *equity-measurement* face (R1–R3 of the grill-resolved slice stack) **SHIPPED 2026-06-15 (PR #43, merge `9d42ac5`, 6 WPs)** — discovered already-built during the 2026-07-05 re-grounding after the docs were never flipped at build time — and **prod-VALIDATED 2026-07-05**: the founding case reads correctly (Natalie — 49% of board setup yet ~3% on the old physical-points lens — now leads the planning lanes) and Reduced/Minimal capacity tiers have been exercised by both household adults. Shipped: (1) rename→"Physical board-load — this week"+demote, (2) capacity-aware physical share (`PhysicalCapacityTier` + `ExpectedSharePct`), (3) planning footprint lanes (un-blended labeled tallies, no exchange rate). **Open remainder: (4) capacity-aware suggestion mechanism [R4]** — deferred; grill with the framing test in `data/outputs/reviews/fca-phase15-equity-regrounding/REGROUNDING-MEMO.md` when prioritized. Probe deferred the MealPlanEntry creator migration (no meal-plan signal). KB: `chores-equity-rework-phase-15-grill-resolved-prod-probe-verdict-build`.
...
### Phase 15: Chores v1.5 — Equity rework / invisible labor (MEASUREMENT SLICE SHIPPED + VALIDATED; R4 open) + chore-history surface (MERGED)
...
| 15. Chores v1.5 (Equity rework / invisible labor) | 6 WPs (R1–R3) | Measurement slice Complete + validated; R4 open | **PR #43 (2026-06-15, `9d42ac5`)** — prod-validated 2026-07-05 (Natalie leads planning lanes; Reduced/Minimal tiers exercised); **history surface MERGED — PR #70 (2026-07-05)**; R4 (capacity-aware suggestion) deferred |
...
---
*Created: 2026-01-22*
*Last updated: 2026-07-05 (Phase 15 chore-history surface — ledger + logbook — MERGED + DEPLOYED via the 9-WP `fca-chore-history` spec, PR #70 → master `8d738da`; CI + Deploy both green. Also reconciled Phase 16 Snooze to SHIPPED — merged pre-flip via PR #47, carried into the SPA by de-Blazor flip #65, prod-confirmed. Also reconciled the Phase 15 equity-measurement slice to reality: R1–R3 SHIPPED 2026-06-15 as PR #43 — discovered during the Fable re-grounding (`data/outputs/reviews/fca-phase15-equity-regrounding/REGROUNDING-MEMO.md`) after the docs/quest were never flipped at build time — and prod-VALIDATED 2026-07-05 (founding case + capacity tiers exercised); R4 is the open remainder. Trust the Spine campaign "Family Coordination App" / quests `598a1d49` (done) + `38c77cc4` (R4) over this snapshot for live work-state.)*
