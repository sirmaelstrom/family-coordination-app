...
- **Non-empty /api error bodies** — any 4xx written on `/api` must carry a body, or `UseStatusCodePagesWithReExecute` re-executes it through the GET-only `/not-found` page and a non-GET call surfaces as 405 (auto-memory `fca-local-browser-verify-recipe` — *pointer corrected 2026-08-08: this cited `fca-empty-404-surfaces-as-405-on-delete`, which stopped resolving when the 2026-08-05 consolidation absorbed it into that note*).
