...
must_haves:
  ...
  key_links:
    ...
    - from: "IngredientEntry.razor"
      to: "IngredientParser"
      via: "DI injection"
      pattern: "@inject IngredientParser"
---
...
Output: IngredientParser service with comprehensive tests covering common ingredient patterns, registered in DI container.
</objective>
...
<task type="auto">
  <name>REFACTOR: Register IngredientParser in DI and clean up if needed</name>
  <files>
    src/FamilyCoordinationApp/Program.cs
    src/FamilyCoordinationApp/Services/IngredientParser.cs
  </files>
  <action>
1. Register IngredientParser in Program.cs (before `var app = builder.Build()`):
   ```csharp
   builder.Services.AddScoped<IngredientParser>();
   ```

2. Review the parser implementation and refactor if any obvious improvements exist:
   - Verify no code duplication
   - Ensure method names are clear
   - Consider adding XML documentation comments for public API
   - Verify regex patterns are correct and performant (using source generators)
...
4. Commit changes: `refactor(02-02): register IngredientParser in DI`
  </action>
  <verify>
    `dotnet test tests/FamilyCoordinationApp.Tests` runs and ALL tests PASS
  </verify>
  <done>IngredientParser registered in DI, parser refactored (or skipped if no improvements needed), tests pass</done>
</task>
...
4. IngredientParser registered as scoped service in Program.cs
5. All tests pass with `dotnet test`
6. Parser handles: simple quantities, fractions, mixed fractions, Unicode fractions, ranges, notes, unitless
</verification>

<success_criteria>
- RED phase: All tests written and failing
- GREEN phase: All tests passing
- REFACTOR phase: Service registered in DI, code is clean
...
- IngredientParser can be injected into Blazor components
</success_criteria>
