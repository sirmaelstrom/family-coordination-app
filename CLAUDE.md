...
- **Non-empty /api error bodies** — now guaranteed by the pipeline, not by discipline: `/api` is branched past `UseStatusCodePagesWithReExecute` to `UseStatusCodePages`, which backfills a JSON `{message}` on any bodiless 4xx and preserves the status (PR #90; guard `ApiErrorBodyTests`). Still write your own message — the backfill is deliberately generic. **Not covered:** the exception path (a failed `[FromBody]` bind, an unhandled throw) still answers text-plain/HTML — quest `ea816df2`.
